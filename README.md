...
local searches. Marktplaats also has native saved-search hunts that work without partner API access.
Buying always stays on the original marketplace and always remains manual.
...
- Marktplaats works without API credentials through explicit native saved-search handoffs. Deal Shark
  opens at most 100 high-value wantlist searches, one click at a time, with a strict price ceiling;
  Marktplaats itself handles the notifications after you save the search.
- If official OAuth2 credentials are later assigned by Marktplaats, the existing automatic,
  pressing-verified scanner remains available. Neither mode scrapes Marktplaats pages.
