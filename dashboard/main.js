...
const { createMarktplaatsClient, createMarktplaatsWebClient } = require('./marktplaats/client');
const { createMarktplaatsService } = require('./marktplaats/service');
const { runAllScans, waitForIdle } = require('./all-scan');
...
// Releases whose last Discogs observation showed nothing for sale. Both Vinted and Marktplaats
// hunt these first: they are the titles a listing is most likely to be worth acting on quickly.
async function loadRareTargets() {
  try {
...
async function testMarktplaatsConnection() {
  try {
    ...
    const client = hasCredentials ? createMarktplaatsClient(credentials) : createMarktplaatsWebClient();
    const status = await client.health();
    ...
    return { ok: true, ...status, mode: client.mode, sampleItems: sample.items.length, total: sample.total };
  } catch (error) {
    return { ok: false, error: error && error.message ? error.message : String(error), status: error && error.status || null };
  }
}
...
async function runAllMarketplaceScans(win) {
  ...
  try {
    ...
    const runner = (run, { unavailableReason = null, detail = null } = {}) => (
      unavailableReason ? { unavailableReason } : { run, ...(detail ? { detail } : {}) }
    );
    ...
    const runServiceWhenIdle = async (service, initialSnapshot, run, { reuseFullScan = false, label = 'Marketplace scan' } = {}) => {
      const initiallyRunning = !!(initialSnapshot.status && initialSnapshot.status.running);
      const currentRunIsFull = !!(initialSnapshot.status && initialSnapshot.status.progress && initialSnapshot.status.progress.all);
      if (initiallyRunning) {
        await waitForIdle({ isRunning: () => !!service.snapshot().status.running, label });
        if (reuseFullScan && currentRunIsFull) return runService(() => service.snapshot());
      }
    ...
    return await runAllScans({
      runners: {
        discogs: runner(
          async () => {
            if (scrapeRunning) await waitForIdle({ isRunning: () => scrapeRunning, label: 'Discogs scan' });
            return runScrape(win, { fullMedians: true, coordinated: true });
          },
          {
            unavailableReason: !config.username || !config.token ? 'account setup needed' : null,
            detail: scrapeRunning ? 'waiting for current scan' : null,
          },
        ),
        vinted: runner(
          () => runServiceWhenIdle(vinted, vintedSnapshot, () => vinted.runOnce({ forceDeep: true }), { label: 'Vinted scan' }),
          {
            unavailableReason: !config.username || !config.token ? 'Discogs setup needed' : null,
            detail: vintedSnapshot.status && vintedSnapshot.status.running ? 'waiting for current scan' : null,
          },
        ),
        ebay: runner(
          () => runServiceWhenIdle(ebay, ebaySnapshot, () => ebay.runOnce({ all: true }), { reuseFullScan: true, label: 'eBay scan' }),
          {
            unavailableReason: !(ebaySnapshot.status && ebaySnapshot.status.configured) ? 'API setup needed' : null,
            detail: ebaySnapshot.status.running ? 'waiting for current scan' : null,
          },
        ),
        tradera: runner(
          () => runServiceWhenIdle(tradera, traderaSnapshot, () => tradera.runOnce({ all: true }), { reuseFullScan: true, label: 'Tradera scan' }),
          {
            unavailableReason: !(traderaSnapshot.status && traderaSnapshot.status.configured) ? 'API setup needed' : null,
            detail: traderaSnapshot.status.running ? 'waiting for current scan' : null,
          },
        ),
        marktplaats: runner(
          () => runServiceWhenIdle(marktplaats, marktplaatsSnapshot, () => marktplaats.runOnce({ all: true }), { reuseFullScan: true, label: 'Marktplaats scan' }),
          {
            unavailableReason: !(marktplaatsSnapshot.status && marktplaatsSnapshot.status.configured) ? 'partner access needed' : null,
            detail: marktplaatsSnapshot.status.running ? 'waiting for current scan' : null,
          },
        ),
      },
      onUpdate: send,
